...
public class BookingService : IBookingService
{
    ...
    public Booking Book( int userId, string categoryName, DateTime startDate, DateTime endDate, Currency currency )
    {
        ...
        {
            throw new ArgumentException( "End date cannot be earlier than today" );//проверка, что дата брони не раньше, чем сегодня
        }
        ...
        if ( endDate == startDate )
        {
            throw new ArgumentException( "Start date cannot be equal to end date" ); //дата выезда не может быть равна дате заезда
        }
    ...
    public decimal CalculateCancellationPenaltyAmount( Booking booking )
    {
        ...
        int daysBeforeArrival = ( booking.StartDate - DateTime.Now ).Days + 1; //изменила подсчет дней
    ...
    private static decimal CalculateBookingCost( decimal baseRate, int days, int userId, decimal currencyRate )
    {
        ...
        decimal totalCost = ( cost - cost * CalculateDiscount( userId ) ) / currencyRate; //изменила подсчет стоимости
        return totalCost;
    }
}
