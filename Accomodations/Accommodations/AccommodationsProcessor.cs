...
public static class AccommodationsProcessor
{
    ...
    private static void ProcessCommand( string input )
    {
        ...
        switch ( commandName )
        {
            case "book":
                ...
                if ( !Enum.TryParse( parts[ 5 ], true, out CurrencyDto currency ) ) //при неправильной валюте выбрасывается исключение
                {
                ...
                BookingDto bookingDto = new()
                {
                    ...
                    Category = parts[ 2 ],
                    StartDate = ParseStringToDate( parts[ 3 ] ), //функция с выбрасыванием исключения
                    EndDate = ParseStringToDate( parts[ 4 ] ),
                    Currency = currency,
                };
            ...
            case "undo":
                ...
                {
                    Console.WriteLine( "No commands to undo" ); //проверка на пустой список команд
                    return;
                }
            ...
            case "search":
                ...
                DateTime startDate = ParseStringToDate( parts[ 1 ] ); //функция с выбрасыванием исключения
                DateTime endDate = ParseStringToDate( parts[ 2 ] );
    ...
    private static DateTime ParseStringToDate( string str ) //функция для перевода строки в дату
    {
